...
const Card = ({ onSave, villeOptions }) => {
  ...
  return (
    <div className="card">
      ...
      <button className="card-save-button" onClick={handleSave}>Save</button>
    </div>
  );
};
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item.id}>
              ...
              <td>
                <button className="card-update-button" onClick={() => handleUpdate(item.id)}>Update</button>
                <button className="list-view-delete-button" onClick={() => onDelete(item.id)}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
...
function Zone() {
  ...
  return (
    <div className="app">
      ...
      <ListView items={listItems} onDelete={handleDeleteItem} />
    </div>
  );
}
